<template>
    ...
    <div v-else v-loading="isLoading" class="b bg-white mx-auto receipt-container max-w-[7.9cm] w-full rounded-lg overflow-hidden text-xs">
        ...
        <div class="pt-6 pb-4 flex flex-col items-center">
            ...
            <h1 class="text-sm font-bold text-gray-800 leading-4">ADIYASA MEDIKA</h1>
            <p class="text-xs text-gray-600 mt-1 px-8 text-center leading-4">Jl. Raya Adiyasa, Kec. Solear, Kabupaten Tangeranga, Banten</p>
        </div>
...
import { apiPublicReceipt } from '../../../api/salesApi';
...
function firstLoad() {
    if (!trxId.value) {
        ...
        getData(() => apiPublicReceipt(trxId.value), true, true);
    }
}
